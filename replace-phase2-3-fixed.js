...
// 옛날 2차 배치 시작 (1066줄)부터 "최종 배정 완료" 앞까지 제거
// "주4일 최소 보장 완료" 다음부터 "형평성 편차 저장 시작" 전까지를 교체

// 교체할 부분 찾기
const oldPhase2Start = lines.findIndex((line, i) => i >= 1060 && line.includes('새로운 2차 배치 로직'));
const fairnessStart = lines.findIndex((line, i) => i >= 1200 && line.includes('형평성 편차 저장 시작'));

console.log(`옛날 Phase 2 시작: ${oldPhase2Start + 1}줄`);
console.log(`형평성 저장 시작: ${fairnessStart + 1}줄`);

// 새로운 Phase 2 + Phase 3 코드 (옛날 2차 배치를 주석처리하고 새로운 2/3차 추가)
const newPhase2And3 = `      // ==================== 옛날 2차 배치 (비활성화) ====================
      /*
      // 날짜별 OFF 균등 분배 로직 (사용하지 않음)
      */
...
// 옛날 Phase 2 부분 제거하고 새로운 코드 삽입
const beforePhase2 = lines.slice(0, oldPhase2Start).join('\n');
const afterPhase2 = lines.slice(fairnessStart).join('\n');
...
console.log(`\n변경 내용:`);
console.log(`- 기존 라인 ${oldPhase2Start + 1}~${fairnessStart} 제거`);
