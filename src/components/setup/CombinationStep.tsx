...
interface Combination {
  ...
  departmentCategoryStaff?: {
    [departmentName: string]: {
      [categoryName: string]: {
        count: number        // 필요 인원
        minRequired: number  // 꼭 필요 인원 (0이면 필수 아님)
      }
    }
  }
  doctors: string[]
...
export function CombinationStep({
  ...
  interface DoctorOption {
    displayName: string  // 화면 표시용: "박원장(상담)"
    shortName: string    // 조합명용: "박(상담)"
  }
  ...
  // 선택된 의사들로부터 조합명 자동 생성 (shortName 사용)
  ...
  // 부서별 필요 인원이 변경되면 자동으로 구분별 인원 계산
  useEffect(() => {
    ...
    departments
      ...
      .forEach(dept => {
        ...
        Object.entries(categoryCount).forEach(([catName, count]) => {
          ...
          // 기존 값이 있고 count가 동일하면 minRequired 유지
  ...
  const downloadTemplate = () => {
    // 저장된 조합이 있으면 실제 데이터 사용, 없으면 예제 사용
    const templateData = data.length > 0
      ? data.map((combination) => {
          ...
          const row: any = {
            조합명: combination.name,
            요일: dayLabel,
            필요인원: combination.requiredStaff,
            구분: combination.hasNightShift ? '야간' : '',
          }

          // 의사 정보 추가 (최대 10명)
          combination.doctors.forEach((doctor, index) => {
            row[`의사${index + 1}`] = doctor
          })
    ...
    const doctorListData = doctorOptions.length > 0
      ? doctorOptions.map(opt => ({ 의사명: opt.displayName, 조합명표시: opt.shortName }))
      : [{ 의사명: '원장 정보를 먼저 입력해주세요', 조합명표시: '-' }]
        ...
        jsonData.forEach((row, index) => {
          ...
          for (let i = 1; i <= 10; i++) {
            ...
            if (row[doctorKey] && String(row[doctorKey]).trim()) {
              ...
              // 의사 존재 여부 확인 (displayName 또는 shortName 허용)
    ...
    <div className="space-y-6">
                            ...
                            👥 필요인원: {
                              Object.keys(combination.departmentRequiredStaff || {}).length > 0
                                ? Object.entries(combination.departmentRequiredStaff)
                                    ...
                                    .join(', ')
                                : `${combination.requiredStaff}명 (전체)`
                            }
                          </span>
                ...
                {isExpanded && hasDeptStaff && (
                  <div className="p-4 border-t border-gray-200 bg-white space-y-4">
                    <div className="flex items-center justify-between mb-2">
                      <Label className="text-sm font-semibold text-blue-700">구분별 상세 인원</Label>
                    ...
                    {(!combination.departmentRequiredStaff || Object.keys(combination.departmentRequiredStaff).length === 0) ? (
                      <div className="p-4 bg-blue-50 border border-blue-200 rounded text-center">
                        <p className="text-sm text-blue-900">
                          위의 "자동 계산" 버튼을 눌러 부서별/구분별 인원을 설정해주세요.
                        </p>
                      </div>
                    ) : (
                      departments
                        ...
                        return (
                          <div key={dept.name} className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                            <h4 className="font-semibold text-sm mb-3">{dept.name} (총 {deptRequired}명)</h4>
                              ...
                              {deptCategories.map(cat => {
                                ...
                                return (
                                  <div key={cat} className="grid grid-cols-[120px_1fr_1fr] gap-4 items-center p-2 bg-white rounded">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Label className="text-xs text-gray-500 w-12">인원:</Label>
                                      <Input
                                        type="number"
                                        min={0}
                                        max={deptRequired}
      ...
      {!editMode && (
        ...
        <div className="space-y-4">
          ...
          {/* 부서별 필요 인원 */}
          <div className="space-y-2">
            <Label className="text-xs font-semibold">부서별 필요 인원 *</Label>
            ...
            {departments.filter(d => d.useAutoAssignment).length === 0 && (
              <p className="text-xs text-amber-700 bg-amber-50 p-2 rounded border border-amber-200">
                ⚠️ 자동배치를 사용하는 부서가 없습니다. 2단계에서 설정해주세요.
              </p>
            )}
          </div>
          ...
          {Object.entries(newCombination.departmentRequiredStaff).some(([dept, count]) =>
            ...
            <div className="space-y-3">
              <Label className="text-sm font-semibold text-blue-700">구분별 상세 인원 (자동 계산됨)</Label>
              ...
              <p className="text-xs text-gray-600 bg-blue-50 p-2 rounded">
                💡 <strong>필수 인원:</strong> 체크하면 자동배치 시 반드시 배치되어야 하는 최소 인원입니다.
              </p>
            </div>
          )}
          ...
          {/* 의사 선택 - 버튼 방식 */}
          <div className="space-y-2">
            <Label className="text-xs">의사 선택 * (클릭하여 선택/해제)</Label>
            ...
            {newCombination.doctors.length > 0 && (
              <div className="text-xs text-gray-600 p-2 bg-blue-50 rounded border border-blue-200">
                선택됨: {newCombination.doctors.map(d => getDisplayName(d)).join(', ')}
              </div>
            )}
          </div>
