...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const dateKey of allDatesInRange) {
      ...
      const dayType = getDayType(currentDate, hasNightShift, holidayDates, closedDays)
    ...
    for (const dateKey of sortedDates) {
      ...
      // 나머지 진료실 직원은 OFF로 저장 (주4일 제한 전의 모든 직원 대상)
      ...
      console.log(`   ✅ ${dateKey} 배정 완료: 근무 ${assignedStaff.length}명, OFF ${offStaff.length}명`)
    }
    ...
    for (const weekKey of Array.from(allWeekKeys).sort()) {
      ...
      for (const { staff, workDays } of staffBelowMinimum) {
        ...
        // 이 주차의 날짜 중 해당 직원이 OFF인 날짜 찾기
        const offDates: Array<{
          date: Date
          dateKey: string
          dayType: DayType
          hasNightShift: boolean
          ...
          if (sortedDates.includes(dateKey)) {
            ...
            if (assignment) {
              ...
              if (doctorSchedule) {
                ...
                const dayType = getDayType(new Date(currentDate), hasNightShift, holidayDates, closedDays)
        ...
        if (offDates.length === 0) {
          console.log(`      - ${staff.name}: OFF 날짜 없음 (추가 배치 불가)`)
          continue
        }
        ...
        // OFF 날짜를 형평성 기반으로 정렬
        // 우선순위: 1. 각 날짜 유형별 형평성 점수 2. 날짜 순서
        offDates.sort((a, b) => {
          // 각 날짜 유형에 대응하는 형평성 deviation 계산
          const getDeviation = (dayType: DayType) => {
            switch (dayType) {
              case 'NIGHT': return staffFairness.dimensions.night.deviation
              case 'WEEKEND': return staffFairness.dimensions.weekend.deviation
              case 'HOLIDAY_ADJACENT': return staffFairness.dimensions.holidayAdjacent.deviation
              case 'NORMAL': return staffFairness.dimensions.total.deviation
            }
          }

          const deviationA = getDeviation(a.dayType)
          const deviationB = getDeviation(b.dayType)

          // 1순위: 형평성 점수가 더 큰 날짜 유형 우선 (양수 = 덜 일함 = 배치 필요)
          // 예: 야간 deviation이 +2.0, 주말 deviation이 +0.5 → 야간 날짜 우선 선택
          if (Math.abs(deviationA - deviationB) >= 0.1) {
            return deviationB - deviationA // 내림차순 (큰 deviation 우선)
          }
        ...
        console.log(`      - ${staff.name}: OFF 날짜 ${offDates.length}일 중 ${Math.min(needed, offDates.length)}일 선택 (형평성 기준 정렬)`)

        // 필요한 만큼 OFF를 근무로 변경
        const datesToChange = offDates.slice(0, needed)

        for (const { date, dateKey, hasNightShift } of datesToChange) {
          // OFF를 DAY/NIGHT로 변경
          ...
          totalAssignments++
          console.log(`         → ${dateKey}: OFF → ${hasNightShift ? 'NIGHT' : 'DAY'}`)
        }
      }
    }
