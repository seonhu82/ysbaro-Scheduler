  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const dashboardStats: DashboardStats = {
        ...
        attendance: {
          todayCheckIns: attendanceData.success ? attendanceData.data.summary?.totalCheckedIn || 0 : 0,
          todayCheckOuts: attendanceData.success ? attendanceData.data.summary?.totalCheckedOut || 0 : 0,
          currentlyPresent: attendanceData.success ? attendanceData.data.summary?.currentlyPresent || 0 : 0,
          suspicious: attendanceData.success ? attendanceData.data.summary?.suspiciousCount || 0 : 0,
          checkInRate: attendanceData.success ? attendanceData.data.summary?.checkInRate || '0.0' : '0.0'
        },
        leaves: {
        ...
        <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => router.push('/attendance/history')}>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-gray-600 flex items-center gap-2">
              ...
              오늘 출퇴근 ({stats.attendance.checkInRate}%)
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-green-600">{stats.attendance.currentlyPresent}</div>
            <p className="text-xs text-gray-500 mt-1">출근 {stats.attendance.todayCheckIns} / 퇴근 {stats.attendance.todayCheckOuts}명</p>
